...
exports.youtube_video_to_link = function(context, data, _w, callback) {
	var ctx = context[1];
	if(!_w.video_url ||
		(
			_w.video_url.indexOf('www.youtube.com/watch?v=') == -1 &&
			_w.video_url.indexOf('youtu.be/') == -1
		) ||
		!data.youtube_video_to_link
	) return callback([context[0], ctx]);
	
	let videoId = _w.video_url;

	if(videoId.indexOf('youtube.com/watch?v=') > -1) {
		videoId = getParameterByName('v', videoId)
	} else {
		videoId = videoId.replace(/.+\.be\//, '');
	}

	if(!data.youtube_video_to_link[videoId]) return callback([context[0], ctx]);

	let video_info = data.youtube_video_to_link[videoId];
		...
		.on('response', function(response) {
			...
			this.on('end', function() {
				...
				loadImage.onload = () => {
					...
					if(last_video.live == 'live' && _w.show_live_label) drawLiveLabel(ctx, _w);
	...
	startDrawData(video_info)
}



function style_1(_w, last_video, imgHeight, ctx){
	if(_w.show_likes_bar && last_video.done) {
		let grad = '#2693e6';
		let liheHeigth = 4;
	...
	// рисуем лайки
	if(_w.show_likes && last_video.likes) {
	...
	// рисуем дизлайки
	if(_w.show_dislikes && last_video.dislikes) {
		let fontSize = _w.likes_fontSize;
		ctx.font = "400 "+ fontSize +" Arial";
		ctx.fillStyle = _w.likes_color;
		ctx.textBaseline = "top";
...
function style_2(_w, last_video, imgHeight, ctx){
	...
	// рисуем просмотры
	if(_w.show_view && last_video.view) {
		let fontSize = _w.likes_fontSize;
		ctx.font = "400 "+ fontSize +" Arial";
		ctx.fillStyle = _w.likes_color;
		ctx.textBaseline = "top";
...
function style_3(_w, last_video, imgHeight, ctx){
	...
	// рисуем комментарии
	if(_w.show_comments && last_video.comments) {
		let fontSize = _w.likes_fontSize;
		ctx.font = "400 "+ fontSize +" Arial";
		ctx.fillStyle = _w.likes_color;
		ctx.textAlign = 'left';
	...
	if(_w.show_title && last_video.title) {
